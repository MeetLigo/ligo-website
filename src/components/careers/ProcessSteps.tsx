    ...
    <ol className="mt-9 grid gap-x-6 gap-y-9 sm:grid-cols-2 lg:grid-cols-5">
      {process.map((step, i) => (
        <li key={step.title} className="relative">
          ...
          {i < process.length - 1 && (
            <span aria-hidden className="absolute left-[52px] right-[-24px] top-[21px] hidden h-px bg-ink/[0.16] lg:block" />
          )}
          <span className="relative flex h-[42px] w-[42px] items-center justify-center rounded-full border border-ink/[0.18] bg-white/70 text-[#EA580C]">
            <svg width="20" height="20" viewBox="0 0 24 24" aria-hidden>
              {ICONS[i]}
            </svg>
          </span>
          <div className="mt-4 font-serif text-[13px] font-medium tabular-nums text-[#EA580C]">0{i + 1}</div>
          <div className="mt-[3px] font-serif text-[19px] font-medium leading-tight text-ink">{step.title}</div>
          <p className="mt-[6px] text-[14px] leading-[1.55] text-ink/[0.65]">{step.body}</p>
        </li>
      ))}
    </ol>
  );
