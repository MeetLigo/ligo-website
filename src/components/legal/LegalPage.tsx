  ...
  return (
    <main className="animate-riseIn">
      <div className="mx-auto w-full max-w-[760px] px-6 pb-24 pt-40 sm:px-10">
        <h1 className="font-serif text-[clamp(30px,4.5vw,44px)] font-medium leading-[1.05] tracking-[-0.02em] text-[#FAF6EF]">
          {title}
        </h1>
        <p className="mt-3 text-[14px] text-[#FAF6EF]/60">Last updated {updated}</p>
        <div className="legal-prose mt-10 text-[16px] leading-[1.7] text-[#FAF6EF]/[0.82]">{children}</div>
      </div>
    </main>
  );
}
...
export function LegalH2({ children }: { children: ReactNode }) {
  return (
    <h2 className="mt-10 font-serif text-[22px] font-medium leading-[1.2] text-[#FAF6EF] first:mt-0">{children}</h2>
  );
}
